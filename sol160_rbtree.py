# Hacked from http://code.activestate.com/recipes/576817-red-black-tree/
class rbnode(object):
    """
	...
	def prev(self):
		if self.left.key == None:
			...
			x = self
		x = x._left
		while x._right and x._right.key != None:
			x = x._right
		return x
	def next(self):
		if self.right.key == None:
			...
			x = self
		x = x._right
		while x._left and x._left.key != None:
			x = x._left
		return x
